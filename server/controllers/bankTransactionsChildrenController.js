...
// @access Public
const getAllTransactionsByAccountId = asyncHandler(async (req, res) => {
...
// @access Public
const getLastTransactionByAccountId = asyncHandler(async (req, res) => {
    const account_id = req.params.account_id;
    db.select('*')
        .from('v_last_bank_transactions_children')
        .where('account_id', account_id)
...
const postTransaction = asyncHandler(async (req, res) => {
    ...
    const { account_id, transaction_text, transaction_value, type } = req.body;
    ...
    const account_value = Number(lastAccountValue) + Number(transaction_value);
